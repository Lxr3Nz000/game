...
export default function GamePage() {
  ...
  return (
    <div
      ...
      style={{ padding: "10px", paddingBottom: "120px" }}
    ...
    >
      <div className="max-w-[1400px] mx-auto space-y-3 md:space-y-4 relative z-10">
        ...
        {/* Tab nav: horizontal scroll on mobile with icons */}
        <div
        ...
        >
          {TABS.map(({ k, icon: Icon }) => (
            <button
              key={k}
              className={`sm-tab !px-3 md:!px-4 !py-2.5 md:!py-2.5 flex items-center gap-1.5 shrink-0 min-h-[44px] ${tab === k ? "active" : ""}`}
              onClick={() => setTab(k)}
        ...
        <div className="text-center text-[10px] text-[color:var(--sm-text-dim)] py-4 sm-display px-2">
          &gt; valuation: €{derived.valuation.toLocaleString()} · session {Math.floor((Date.now() - state.startedAt) / 60000)}m
        </div>
      </div>
      ...
      {state.bankrupt && (
        ...
        >
          <div
            className="sm-panel p-5 md:p-6 shake"
